...
1. `swift build`
1. `.build/debug/Kitura-Safe-Server`
...
* `docker run -it -p 8080:8080 -v $PWD:/swift-project -w /swift-project myapp-run sh -c .build-ubuntu/release/Kitura-Safe-Server`


#### Kubernetes
To deploy your application to your Kubernetes cluster, run `helm install --name myapp .` in the `/chart/KituraSafeServer` directory. You need to make sure you change the `repository` variable in your `chart/KituraSafeServer/values.yaml` file points to the docker image containing your runnable application.
